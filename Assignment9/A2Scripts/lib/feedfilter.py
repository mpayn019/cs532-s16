...
# Modified so that we can classifiy a certin numer of entries
def read(feed,classifier,numClassify=None):
...
def entryfeatures(entry):
  ...
  summarywords=[s.lower() for s in splitter.split(entry['description']) 
          if len(s)>2 and len(s)<20]
  #print(len(summarywords))
