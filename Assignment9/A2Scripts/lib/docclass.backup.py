...
class classifier:
  ...
  def fcount(self,f,cat):
    #res=self.con.execute('SELECT count FROM feature_category_combinations WHERE feature="{feat}" AND category="{ccat}"'.format(f,cat)).fetchone()
    ss = f.replace("'",'')
    query = 'SELECT count FROM feature_category_combinations WHERE feature="{0}" AND category="{1}"'.format(ss,cat)
    #print(qu)
    #print(query)
